...
void actualiser_tab_rondin_joueur(t_rondin * tableau_rondin, t_joueuur * tableau_joueur) {
    ...
    for (i = 0; i < NOMBRE_TAUPE; i++) {
        if ((tableau_joueur[i].x == tableau_rondin[i].posx) && (tableau_joueur[i].y == tableau_rondin[i].posy)) {
            printf("TAP !\n");

            tableau_joueur[i].affichage = 0;
...
/*int verfication_riv(t_rondin tab_rondin, t_joueuur tab_joueur){
    for (int i = 0; i < 109; ++i) {
        for (int j = 0; j < 50; ++j) {
            if (tab_joueur.x + tab_joueur.tx== tab_rondin.taille[i][j][0]  )
                if (tab_joueur.y + tab_joueur.ty== tab_rondin.taille[i][j][1])
                    return 1;
        }
